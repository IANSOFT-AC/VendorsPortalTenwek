...
class Recruitment extends Component
{
    ...
    public function getProfileID(){
        if(Yii::$app->session->has('HRUSER')){            
            $hruser = User::findByUsername(Yii::$app->session->get('HRUSER')->username);
    ...
    public function HasProfileOnDynamics($IdFromPortalTable){
        $srvc = Yii::$app->params['ServiceName']['SupplierAplicationList'];

        $filter = [
            'PortalId' => $IdFromPortalTable
        ];

        $CompanyData = Yii::$app->navhelper->getData($srvc,$filter);
        // echo '<pre>';
        // print_r($CompanyData);
        // exit;
